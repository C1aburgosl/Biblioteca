...
public class Libro {
	...
	private int ISBN;
	private ListaUsuarioLibro lista;
	
	public Libro(){
		this.autor = "";
		this.titulo = "";
		this.ISBN = 0;
		this.lista = new ListaUsuarioLibro();
	}
	...
	public int buscar(String n){
		...
		ListaUsuarioLibro aux = this.lista;
		NodoUsuarioLibro aux2 = aux.getInicio();
	...
	public boolean prestamo(Usuario user){
		if(this.prestados<this.ejemplares){
			lista.insertarFinal(new NodoUsuarioLibro(user.getNombre()));
			prestados++;
			return true;
		}else{
			return false;
	...
	public String toString(){
		NodoUsuarioLibro aux = lista.getInicio();
		String nombre = "";
		while(aux!=null){
